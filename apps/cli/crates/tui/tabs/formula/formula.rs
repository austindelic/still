...
/// Package source kind rendered by the formula tab.
...
/// Filter for package source kind.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
/// Renderable package row built from formula or cask metadata.
///
/// Rows are normalized for table display so the renderer does not need to know
/// whether the data came from Homebrew formula JSON or cask JSON. Keep this type
/// small and display-oriented; deeper registry metadata should stay in the spec
/// models until the preview pane needs it.
#[derive(Debug, Clone)]
pub struct PackageRow {
    /// Source kind for the package.
    ...
    pub status: String,
    /// Whether the package is currently installed according to cached registry data.
    pub installed: bool,
}
...
impl FormulaTab {
    /// Creates a formula tab by loading packages from the local cache.
    ///
    /// Reads the cached Homebrew formula and cask JSON files from the current
    /// platform cache directory and normalizes them into `PackageRow` values.
    /// Returns an error when cache files exist but cannot be read or parsed as a
    /// JSON array; missing cache files are treated as an empty package list.
    ...
    /// Renders the filtered package table.
    ///
    /// `area` is the table region allocated by the parent layout. `buf` receives
    /// the table widget. `search_query` is applied together with the active kind
    /// and install-status filters. Rendering mutates selection bounds and scroll
    /// offset so the highlighted row stays visible.
    ...
    /// Renders details for the selected package.
    ///
    /// `area` is the preview region and `buf` receives the rendered details.
    /// `search_query` must match the table query so the preview describes the
    /// same filtered row that is highlighted in `render_table`.
    ...
    /// Moves the selected row up or down within the filtered package list.
    ///
    /// `direction` controls whether selection moves toward the previous or next
    /// row. `search_query` is used to compute the active filtered list; selection
    /// wraps at both ends and is unchanged when the filtered list is empty.
    ...
    ///
    /// Call this after any filter or query change that invalidates the current
    /// row index. It keeps table and preview rendering from pointing at stale
    /// rows.
    ...
    /// Advances the package kind filter and resets selection.
    ///
    /// The cycle order is `All -> Formula -> Cask -> All`.
    pub fn cycle_kind_filter(&mut self) {
        self.filters.kind = self.filters.kind.next();
        self.reset_selection();
    }

    /// Advances the install status filter and resets selection.
    ///
    /// The cycle order is `All -> Installed -> Available -> All`.
    pub fn cycle_install_filter(&mut self) {
        self.filters.install = self.filters.install.next();
        self.reset_selection();
    }

    /// Restores all filters to their default state and resets selection.
    ///
    /// After this call both package kind and install status filters are `All`.
    pub fn reset_filters(&mut self) {
        self.filters = PackageFilters::default();
        self.reset_selection();
    }

    /// Returns the number of packages after filters are applied.
    ///
    /// `search_query` is evaluated with the same fuzzy matching path used by the
    /// table renderer, so this count should be used for UI labels and selection
    /// bounds that need to match visible rows.
